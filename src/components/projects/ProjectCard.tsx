...
import { FolderKanban, FileText, Bot } from "lucide-react";
...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <Link
      href={`/${locale}/orgs/${project.orgSlug}?project=${project.slug}`}
    ...
    >
      <div className="flex items-start gap-3 mb-2">
        <div className="w-9 h-9 rounded-lg bg-[var(--surface-muted)] flex items-center justify-center shrink-0">
          <FolderKanban size={16} className="text-[var(--text-muted)]" />
        </div>
        <div className="min-w-0">
          <h3 className="font-semibold text-[var(--text)] group-hover:text-[var(--brand-accent)] transition-colors truncate">
            {project.name}
          </h3>
          <p className="text-xs text-[var(--text-subtle)]">{project.orgName}</p>
        </div>
      </div>
      {project.description && (
        <p className="text-sm text-[var(--text-muted)] line-clamp-2 mb-3">{project.description}</p>
      )}
      <div className="flex items-center gap-4 text-xs text-[var(--text-subtle)]">
        <span className="flex items-center gap-1">
          <FileText size={11} /> {project.documentCount}
        </span>
        <span className="flex items-center gap-1">
          <Bot size={11} /> {project.agentCount}
        </span>
      </div>
    </Link>
  );
